// -----------------------------------------------------------------------------
//  nexus | GenericCircularPhotosensor.cc
//
//  Geometry of a configurable circular-shaped photosensor.
//
...
GenericCircularPhotosensor::GenericCircularPhotosensor(G4String name,
                                       G4double diameter,
                                       G4double thickness):
  GeometryBase        (),
  name_               (name),
...
GenericCircularPhotosensor::~GenericCircularPhotosensor()
{
}
...
void GenericCircularPhotosensor::ComputeDimensions()
{
  ...
  reduced_size_  = diameter_  -  0 * micrometer;
  ...
  if ((window_thickness_ + sensarea_thickness_ + wls_thickness_) > thickness_) {
    G4Exception("[GenericCircularPhotosensor]", "ComputeDimensions()", FatalException,
...
void GenericCircularPhotosensor::DefineMaterials()
{
  // Case /////
  ...
  if (with_wls_coating_) {
    if (window_rindex_)
      G4Exception("[GenericCircularPhotosensor]", "DefineMaterials()", JustWarning,
  ...
  // set by the user. If it is not set, an exception raises.
    ...
    if (!window_rindex_)
      G4Exception("[GenericCircularPhotosensor]", "DefineMaterials()", FatalException,
...
void GenericCircularPhotosensor::Construct()
{
  ComputeDimensions();
  ...
  G4Tubs* case_solid_vol =
    new G4Tubs(name,0.,diameter_/2., thickness_/2., 0., 360.*deg);
  ...
  G4Tubs* window_solid_vol =
    new G4Tubs(name, 0., reduced_size_/2., window_thickness_/2., 0., 360.*deg);
  ...
  G4Tubs* sensarea_solid_vol =
    new G4Tubs(name, 0., reduced_size_/2., sensarea_thickness_/2., 0., 360.*deg);
  ...
  if (with_wls_coating_) {
    ...
    G4Tubs* wls_solid_vol =
      new G4Tubs(name, 0., diameter_/2, wls_thickness_/2., 0., 360.*deg);
  ...
  if (!sensitive_mpt_)
    G4Exception("[GenericCircularPhotosensor]", "Construct()", FatalException,
